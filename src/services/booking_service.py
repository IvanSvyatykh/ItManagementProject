...
from src.config import SERVICE_ACCOUNT_FILE, SCOPES, CALENDAR_ID
...
async def format_event(event: dict) -> str:
    ...
    location = await normalize_location(event.get('location', 'Без указания места'))
    return f"{location} - {start.strftime('%Y-%m-%d %H:%M')} - {end.strftime('%H:%M')}"


async def get_events(start_date: datetime, end_date: datetime) -> list[dict]:
    """
...
async def get_next_event(location: str) -> list[dict]:
    ...
    for event in events_today:
        if location.lower() in (await normalize_location(event.get('location', '')).lower()):
